...
def agregar_libro():
    ...
    try:
        cur.execute("""
            INSERT INTO libros (isbn, titulo, autor, editorial, categoria_id, ubicacion, precio,
...
def ver_inventario():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT l.id_libro, l.isbn, l.titulo, l.autor, c.nombre, l.ubicacion,
               l.cantidad_total, l.cantidad_disponible, l.estado, l.precio
        FROM libros l
...
def reabastecer_biblioteca():
    """
    Aumenta la cantidad disponible de un libro existente
    sin crear uno nuevo.
    """
    conexion = conectar()
    cursor = conexion.cursor()
    ...
    try:
        ...
        # Sumar al total y a los disponibles
...
def buscar_libros():
    ...
    termino = input("Buscar por título o autor: ")
    cur.execute("SELECT * FROM libros WHERE titulo LIKE ? OR autor LIKE ?", (f"%{termino}%", f"%{termino}%"))
    resultados = cur.fetchall()

    if resultados:
        print("\n=== RESULTADOS ===")
        for libro in resultados:
            estado = "Disponible" if libro[3] else "Prestado"
            print(f"{libro[1]} — {libro[2]} ({estado})")
    else:
...
def ver_libros_por_categoria():
 ...
 # Mostrar las categorías disponibles
    ...
    # Mostrar libros de esa categoría
    nombre_categoria = categoria[0]

# Mostrar libros de esa categoría
    cursor.execute("""
    SELECT titulo, autor, editorial, cantidad_disponible
    FROM libros
    WHERE categoria_id = ?
""", (id_categoria,))
    ...
    if not libros:
     print("No hay libros registrados en esta categoría.")
        ...
        for titulo, autor, editorial, cantidad_disponible in libros:
            estado = "Disponible" if cantidad_disponible > 0 else "Agotado"
        print(f"{titulo} — {autor} | {editorial} | {estado}")
...
# Mostrar libros de esa categoría
        cursor.execute("""
            SELECT id_libro, titulo, autor, cantidad_disponible
            FROM libros
            WHERE categoria_id = ?
        """, (categoria_id,))
        libros = cursor.fetchall()
        ...
        if not libros:
            print("No hay libros disponibles en esa categoría.")
            conexion.close()
            return

        print("\nLibros disponibles en esa categoría:")
        for l in libros:
            print(f"{l[0]} — {l[1]} ({l[2]}) — Disponibles: {l[3]}")
 ...
 # Buscar libro por título
        ...
        id_libro, titulo_libro, disponible = libro
        if disponible <= 0:
            print(f"El libro '{titulo_libro}' no está disponible actualmente.")
            conexion.close()
            return

# Verificar cantidad deseada
    try:
